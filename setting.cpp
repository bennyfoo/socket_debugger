...
void Setting::save(const QString& section, const QLineEdit& lineEdit)
{
    QSettings& store = storage();

    store.setValue(section,lineEdit.text());
}

void Setting::lord(const QString& section, QLineEdit& lineEdit)
{

    QSettings& store = storage();

    QString strEdit = store.value(section).toString().trimmed();

    lineEdit.setText(strEdit);
}
